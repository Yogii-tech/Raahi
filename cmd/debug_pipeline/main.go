...
func main() {
	...
	fmt.Println("--- DEBUG: RUNNING FULL PIPELINE ---")
	pickup := "Haldwani"
	dropoff := "takula"
	date := "30/05/2026"
	...
	pipeline := mongo.Pipeline{
		...
		{{Key: "$match", Value: bson.M{
			"route": bson.M{"$all": []interface{}{
				bson.M{"$regex": primitive.Regex{Pattern: "(?i)" + pickup, Options: ""}},
				bson.M{"$regex": primitive.Regex{Pattern: "(?i)" + dropoff, Options: ""}},
			}},
		}}},
