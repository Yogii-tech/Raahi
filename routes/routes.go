...
func RegisterRoutes(r *gin.Engine) {
	...
	rides.Use(middleware.AuthMiddleware(), middleware.GlobalRateLimiter(60, 1*time.Minute))
	{
		...
		rides.PUT("/:rideId/start", controllers.StartRide)
	...
	notifs.Use(middleware.AuthMiddleware(), middleware.GlobalRateLimiter(60, 1*time.Minute))
	{
		...
		notifs.POST("/test-push", controllers.TestPushNotification)
	}
	...
	reviews.Use(middleware.AuthMiddleware(), middleware.GlobalRateLimiter(60, 1*time.Minute))
	{
		...
		// Uses a distinct label so GET /can-review calls don't burn the POST budget.
		reviews.POST("/", middleware.GlobalRateLimiter(5, 1*time.Minute, "reviews:submit"), controllers.SubmitReview)
	...
	admin.Use(middleware.AuthMiddleware(), middleware.AdminOnlyMiddleware(), middleware.GlobalRateLimiter(30, 1*time.Minute))
	{
		...
		admin.GET("/bookings/detail", controllers.AdminGetBookingDetails)
		admin.GET("/drivers", controllers.AdminDrivers)
		admin.POST("/drivers/:driverId/verify", controllers.AdminVerifyDriver)
		admin.DELETE("/drivers/:driverId", controllers.AdminDeleteDriver)
